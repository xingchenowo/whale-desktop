...
```text
WhaleDesktop-Portable-0.3.3-x64.exe   # 单文件便携版，双击直接运行
WhaleDesktop-Setup-0.3.3-x64.exe      # NSIS 安装程序，创建快捷方式
```

安装后的应用名和快捷方式名称为 **WhaleDesktop(v0.3.3)**。两种产物都已经包含 Electron 运行时，目标电脑不需要安装 Node.js、npm 或 DSH。
...
```text
dist-installer\portable\WhaleDesktop-Portable-0.3.3-x64.exe
dist-installer\nsis\WhaleDesktop-Setup-0.3.3-x64.exe
```
...
- 安装版写入安装目录释放出的 `WhaleDesktop(v0.3.3).exe`。如果更换了安装目录，重新勾选一次即可更新注册表路径。
...
| 键 | 默认 | 说明 |
| --- | --- | --- |
| `skin` | `"default"` | 新桌宠首次启动时的默认皮肤；每只桌宠切换后独立保存 |
...
| 托盘菜单 → 开机自启 | 直接写当前用户注册表，指向已编译的 `WhaleDesktop(v0.3.3).exe` |
| 菜单 → 双开桌宠 | 启动或关闭第二个独立桌宠窗口 |
| 菜单 → 皮肤 | 只切换当前桌宠；双开时两个桌宠皮肤各自独立保存 |
...
- **修复水平翻转下菜单按钮与锁图标重叠**：翻转时锁图标原来还停在立绘左侧，正好和新镜像
  过去的汉堡按钮挤在同一个角。现在锁图标也跟着镜像到立绘另一角（不翻转时位置不变），
  两者各占一角。

### 0.3.2 维护版

- **贴边吸附**：立绘/气泡的可见边缘现在能真正贴住屏幕工作区边缘（右下角不再永远差
  十几像素）。做法是渲染端把「窗口边缘 → 可见内容」的留白报给主进程，吸附时窗口按这个
  留白整体探出，可见部分正好压线；换皮肤/改缩放后留白会重算。窗口探出的部分是透明的，
  仍然点击穿透，不会挡住下层窗口。
- **水平翻转**：开启后气泡（含尾巴朝向）跟着立绘一起镜像到人物正前方，气泡里的文字和
  数值保持正向；菜单按钮也跟着换边。原来只镜像立绘、气泡留在原地。
- **托盘菜单**：开机自启状态改为异步读取 + 缓存，重建菜单不再同步调用 `reg.exe`；
  `--autostart` 启动时托盘图标、键盘钩子和余额预热都会延后一两秒，避免开机第一次右键
  托盘出现"转圈"卡顿。
